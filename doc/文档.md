...
github链接：https://github.com/CharlesDDDD/bookstore
...
![E-R图绘制 ](https://github.com/CharlesDDDD/bookstore/blob/main/doc/image/E-R图绘制 .png)
...
邓淳远：seller接口的编写（基本功能）+ 参数化搜索接口（全局索引优化） + Nacos环境配置 + redis查询优化 +测试代码编写与调优+ 实践报告撰写

沈小奇：buyer接口的编写（基本功能）+ 主动/自动取消订单接口+历史订单接口 +测试代码编写与调优+ 实践报告撰写

崔鹏宇：user接口的编写（基本功能）+seller发货接口+buyer收货接口+ 前台 + 语雀知识文档构建
